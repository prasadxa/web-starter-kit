...
router.get("/admin/analytics/enhanced", async (req: Request, res: Response) => {
  if (!requireSuperAdmin(req, res)) return;
  ...
  const [revenueResult] = await db
    .select({ total: sql<number>`coalesce(sum(d.consultation_fee), 0)::float` })
    .from(appointmentsTable)
    .leftJoin(doctorsTable, eq(appointmentsTable.doctorId, doctorsTable.id))
    .where(and(eq(appointmentsTable.status, "completed")));
  ...
  const topDepartmentRows = await db
    .select({
  ...
  const dailyTrendRows = await db
    .select({
      ...
      count: sql<number>`count(*)::int`,
      revenue: sql<number>`coalesce(sum(d.consultation_fee), 0)::float`,
    })
    .from(appointmentsTable)
    .leftJoin(doctorsTable, eq(appointmentsTable.doctorId, doctorsTable.id))
  ...
  const recentAppointments = recentAppointmentRows.map(r => ({
    ...r.appointment,
    createdAt: r.appointment.createdAt instanceof Date ? r.appointment.createdAt.toISOString() : r.appointment.createdAt,
    doctorFirstName: r.doctorUser?.firstName ?? null,
    doctorLastName: r.doctorUser?.lastName ?? null,
    doctorProfileImageUrl: r.doctorUser?.profileImageUrl ?? null,
    doctorSpecialization: r.doctor?.specialization ?? null,
  ...
  res.json({
    ...
    topDoctors,
    topDepartments: topDepartmentRows.map(r => ({ name: r.name || "Unknown", count: r.count })),
    dailyTrends: dailyTrendRows.reverse().map(r => ({ date: r.date, count: r.count, revenue: r.revenue })),
    recentAppointments,
  });
});
